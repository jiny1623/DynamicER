...
def evaluate(
    ...
    for men_query_idx, men_embed in enumerate(
        ...
        for k in joint_graphs:
            ...
            if k > 0:
                ...
                joint_graph["rows"] = np.append(
                    joint_graph["rows"],
                    [n_entities + men_query_idx] * len(men_cand_idxs[:k]),
                )
    ...
    } if use_types else {"dict_embeds": dict_embeds, "dict_index": dict_index}
...
def main(params):
    ...
    entity_dictionary_pkl_path = os.path.join(
        pickle_src_path, "entity_dictionary.picxkle"
    )
    ...
    if not params["only_evaluate"]:
        ...
        if params["path_to_prev_gold"]:
            prev_train_tensor_data_pkl_path = os.path.join(
                params["path_to_prev_gold"], "train_tensor_data.pickle"
            )
            prev_train_processed_data_pkl_path = os.path.join(
                params["path_to_prev_gold"], "train_processed_data.pickle"
            )
            assert (os.path.isfile(prev_train_tensor_data_pkl_path) and
                    os.path.isfile(prev_train_processed_data_pkl_path)
            )
            with open(prev_train_tensor_data_pkl_path, "rb") as read_handle:
                prev_train_tensor_data = pickle.load(read_handle)
            with open(prev_train_processed_data_pkl_path, "rb") as read_handle:
                prev_train_processed_data = pickle.load(read_handle)
    ...
    # add continual case (entity_dict_vecs)
    ...
    if params["path_to_prev_gold"]:
        ...
        for entry in tqdm(entity_dictionary, desc="continual case"):
            ...
            matching_ids = []
            for item in prev_train_processed_data:
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        if use_types:
            ...
            if load_stored_data:
                ...
                if dict_embed_data is not None:
                    ...
                    # TODO: dictionary embedding
                    (
                        train_dict_embeddings,
                        train_dict_index,
                    ) = data_process.embed_and_index(
                        reranker,
                        entity_dict_vecs, # entity description의 token id list
                        encoder_type="candidate",
                        alpha=params["alpha"],
                        n_gpu=n_gpu,
                        force_exact_search=params["force_exact_search"],
        ...
        for step, batch in enumerate(iter_):
            ...
            for m_embed_idx, m_embed in enumerate(mention_embeddings): # batch dim을 iterate
                mention_idx = int(mention_idxs[m_embed_idx]) # mention의 index
                gold_idxs = set(
                    train_processed_data[mention_idx]["label_idxs"][
                        : n_gold[m_embed_idx] # n_gold : 각 train data 당 mention gold 개수 (우린 항상 1)
                    ]
                ) # 실제 gold entity의 index

                # TEMPORARY: Assuming that there is only 1 gold label
                assert n_gold[m_embed_idx] == 1


                # if ~ else : gold_links (MST) 설계하는 과정
                if mention_idx in gold_links:
                    ...
                    #   positive edge
                    rows, cols, data, shape = ( # edge에 대한 기록
                        [],
                        [],
                        [],
                    ...
                    sim_order = 1 if params["farthest_neighbor"] else -1 # False
                    ...
                    for cluster_ent in gold_idxs:
                        ...
                        if gold_arbo_knn is not None: # None
            ...
            # index를 entity_dict_vecs의 vector로 바꿔주는 작업
            negative_dict_inputs = list(map(lambda x: entity_dict_vecs[x], negative_dict_inputs))
            ...
            for pos_idx in positive_idxs:
                if pos_idx < n_entities:
                    pos_embed = reranker.encode_candidate(
                        entity_dict_vecs[pos_idx : pos_idx + 1],
                        requires_grad=True,
                    )
                else:
            ...
            data_parallel_batch_size_check = (
                ...
                and len(negative_dict_inputs) >= n_gpu
            )
            ...
            if skipped > 0 and not params["within_doc_skip_strategy"]:
                ...
                data_parallel_batch_size_check = (
                    len(skipped_negative_dict_inputs) >= n_gpu
                )
                if data_parallel_batch_size_check:
